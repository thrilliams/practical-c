...
void test_collision_gets_and_sets() {
    hashtable *ht = ht_make(1);  // guaranteeing collisions
...
void test_set_after_remove() {
    ...
    assert_msg(ht_len(ht) == 0 && bo_get == false && bob_get == false,
        "removing all keys should work");
    
    ht_set(ht, "Catamaran", 10);
    int c_val = 0;
    bool c_get = ht_get(ht, "Catamaran", &c_val);

    assert_msg(ht_len(ht) == 1 && c_get == true && c_val == 10,
        "adding a key after all were removed should work");
    
    ht_free(ht);
    pass("setting after removal");
}
